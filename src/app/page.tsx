...
function getDisabledReason({
  ...
  if (nights <= 0) return 'チェックアウト日はチェックイン日の翌日以降を選んでください。';
  return '';
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    getPlanAvailabilityForStay(stay.checkIn, stay.checkOut).then((items) => {
      ...
      setAvailabilityMessage(
        totalAvailable > 0
          ? `選択日程で予約可能な残り枠: ${totalAvailable}`
          : '選択日程は満室です。日程を変更してお試しください。',
      );
    });
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f8fbff_0%,#ffffff_30%,#f7fbf7_100%)]">
      <div className="mx-auto max-w-2xl px-4 py-6">
        <section className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-4 text-sm leading-7 text-red-700">
          <p>カレンダーの空き表示は、実際の予約状況に合わせて自動で更新されます。</p>
          <p>満枠の日は「× / 満室」、残りが少ない日は「△ / 残りわずか」で表示されます。</p>
        </section>
        ...
        <section className="mb-6 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {availabilityMessage && <p className="mt-3 text-sm font-medium text-emerald-700">{availabilityMessage}</p>}
        </section>

        <section className="mb-6 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-base font-semibold text-slate-800">利用人数</h2>
          <div className="space-y-3">
            <Counter label="大人（中学生以上）" value={stay.adults} onChange={(value) => setStay({ adults: value })} min={1} max={20} />
            <Counter label="子ども" value={stay.children} onChange={(value) => setStay({ children: value })} min={0} max={20} />
            <Counter label="幼児" value={stay.infants} onChange={(value) => setStay({ infants: value })} min={0} max={20} />
          </div>
        </section>

        {stayDates.length > 0 && (
          <section className="mb-8">
            <h2 className="mb-3 text-base font-semibold text-slate-800">宿泊日に参加できるイベント</h2>
                ...
                <div key={date} className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  {items.length === 0 ? (
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {items.map((event) => (
                        <article key={event.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                          <img
        ...
        <section className="mb-8">
          ...
          {!canProceed && <p className="mt-2 text-center text-xs text-slate-500">{disabledReason}</p>}
        </section>
      </div>
    </div>
  );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              満枠の日は「× / 満室」、残りが少ない日は「△ / 残りわずか」で表示します。
            </p>
          </div>
      ...
      <div className="px-3 py-4 sm:px-5">
        ...
        <div className="grid grid-cols-7 gap-1">
          {gridDates.map((date) => {
            ...
            const remainingSites = day?.availableSites ?? 0;
            const bookable = Boolean(day?.isBookable) && !outOfRange && remainingSites > 0;
            const isSelected = selectedCheckIn === isoDate;
            const isLowStock =
              bookable && (day?.capacity ?? 0) > 0 && remainingSites / (day?.capacity ?? 1) < 0.1;
            const statusLabel =
              outOfRange || !day?.isBookable
                ? '不可'
                : !bookable
                  ? '満室'
                  : isLowStock
                    ? '残りわずか'
                    : `残${remainingSites}`;
            const mark = outOfRange || !day?.isBookable ? '―' : !bookable ? '×' : isLowStock ? '△' : '○';
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-slate-500">
          ...
          <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-500">× 満室 / 受付外</span>
        </div>

        <p className="mt-3 text-xs leading-5 text-slate-500">
          日付を押すとチェックイン日として選択され、チェックアウト日は翌日に自動設定されます。
        </p>
      </div>
    </section>
  );
